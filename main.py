...
from src.metrics import NOSE_TIP, average_ear, mouth_aspect_ratio
...
def _process_face(frame, result, idx, opts, states, now_s):
    ...
    return f"Face {idx}", lines, record, level
...
def run_image(args, model_path: Path) -> int:
    ...
    for idx, face in enumerate(result.face_landmarks):
        ...
        title, lines, rec, lvl = _process_face(img, result, idx, opts, states, 0.0)
    ...
    with FaceMeshDetector(
        ...
        while True:
            ...
            for idx, face in enumerate(result.face_landmarks):
                ...
                title, lines, rec, lvl = _process_face(frame, result, idx, opts, states, now_s)
                panels.append((title, lines))
                records.append(rec)
                level = _worse(level, lvl)
